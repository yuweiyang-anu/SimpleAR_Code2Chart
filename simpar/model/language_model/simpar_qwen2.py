...
########################################################################################
# Simpar Process:
# JSON data (caption + <|vtokens|> prompt)
#         ↓
# tokenizer → input_ids
#         ↓
# Init accepted_tokens = caption
# Init draft_tokens = [PAD, PAD, PAD, ..., PAD]  (max_steps)
#         ↓
# [Loop] each iter：
#  ├─ merge sequence = accepted_tokens + draft_tokens
#  ├─ Input SimpARForCausalLM（+Vision）
#  ├─ obtain draft logits
#  ├─ softmax → draft_probs
#  ├─ sample draft tokens
#  ├─ (first time)：directly select the first token
#  ├─ (then)：
#  │    ├─ calculate acceptance probability α
#  │    ├─ accept/reject
#  │    ├─ first rejection → resample local tokens
#  ├─ update accepted_tokens
#  └─ check whether done
# obtain the whole visual token sequence
# feishu document: https://ycnsf1dn4rha.feishu.cn/wiki/E060wq0tNiNHi8kMjZpcWX9mnUe
########################################################################################
...
class SimpARForCausalLM(Qwen2ForCausalLM):
    ...
    def generate_visual_sjd(
        ...
        tokenizer,
        **kwargs,
    ) -> Union[GenerateOutput, torch.LongTensor]:  # Speculative Decoding
